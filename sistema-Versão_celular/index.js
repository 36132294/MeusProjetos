...
//variaveis que resgata valores de entrada !!extremamente importante para execulção das outras variaveis.
...
function fu_valorDoboleto (){
    ...
    var resu_valorBo = parseFloat(valorBole);
    return resu_valorBo.toLocaleString("pt-BR", {style:"currency", currency:"BRL"}); 
}
...
function fu_valorMulta(){
  ...
  return resu_valorMulreais.toLocaleString('pt-BR', {style:'currency', currency:'BRL'}) 
} 
...
function fu_vjurosEmDinheiro (){
  ...
  var resu_quantidadeJD;
   ...
   return resu_valorJuEmDinheiro.toLocaleString('pt-BR', {style:'currency', currency:'BRL'});
}
...
function fu_valorBoComvalorMo (){
  ...
  var resu_valorboComvalorMo;
   ...
   return resu_valorboComvalorMo.toLocaleString('pt-BR', {style:'currency', currency:'BRL'}); 
  }
  ...
  var resu_valorboComvalorMo;
   ...
   valorMul = parseInt(valorMul);//valor da multa.
   valorBole = parseFloat(valorBole);//valor do boleto 
   resu_valorMulreais = parseFloat(valorBole * valorMul)/100;//valor do boleto vez valor da multa divido por 100.
   valorBole = parseFloat(valorBole); 
   valorMor = parseFloat(valorMor);
   dataVenc = new Date (dataVenc);//data de vencimento
   dataPag = new Date (dataPag);//data de pagamento
   ...
   dif_tempo = Math.abs(dataPag - dataVenc); //calculo para se obter a diferença de tempo entre duas datas. 
   dife_dias = Math.ceil(dif_tempo / umDia);//calculo para se obter a diferença de dias entre duas datas. 
   resu_quantidadeJD = parseFloat(dife_dias * valorMor);//calculo para se obter a quantidade de juros baseada nos dias em atraso
   resu_valorJuEmDinheiro = parseFloat(resu_quantidadeJD * valorBole)/100;//calculo para se converter o juros em dinheiro.
   resu_valorboComvalorMo = parseFloat(resu_valorJuEmDinheiro + valorBole)//valor do voleto mais o valor da mora ja somada os dias.
   resu_valorTotal = parseFloat(resu_valorboComvalorMo + resu_valorMulreais);//valor do boleto mais o valor da mora mais o valor da multa.
   return resu_valorTotal.toLocaleString('pt-BR', {style:'currency', currency:'BRL'});
  }
